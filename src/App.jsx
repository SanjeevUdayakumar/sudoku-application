...
const App = () => {
  ...
  const renderCell = (row, col) => {
    ...
    const isPrepopulated = value !== null; // Differentiate pre-populated cells
    ...
    return (
      <input
        ...
        className={`sudoku-cell rounded-sm border border-gray-300 px-2 py-1 text-center focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors[`grid[${row}][${col}]`] ? 'border-red-500' : ''}`}
      />
    );
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="sudoku-grid grid grid-cols-9 gap-2">
        {grid.map((row, rowIndex) => (
          <div key={rowIndex} className="sudoku-row grid grid-cols-3 gap-1">
